...
function PureArtifact({
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      if (!dragRef.current || !windowWidth) {
        return;
      }
      const deltaX = e.clientX - dragRef.current.startX;
      const deltaRatio = deltaX / windowWidth;
      const newRatio = Math.min(
        Math.max(dragRef.current.startRatio + deltaRatio, 0.25),
        0.75
      );
        ...
        >
          ...
          {!isMobile && (
            <div
              ...
              className="fixed top-0 z-50 h-dvh w-2 cursor-col-resize select-none"
              onKeyDown={(e) => {
                if (e.key === "ArrowLeft") {
                  setSplitRatio((r) => Math.max(0.25, r - 0.05));
                } else if (e.key === "ArrowRight") {
                  setSplitRatio((r) => Math.min(0.75, r + 0.05));
                }
              }}
              ...
              tabIndex={0}
            >
              <div className="absolute top-1/2 left-1/2 h-8 w-1 -translate-x-1/2 -translate-y-1/2 rounded-full bg-zinc-300 opacity-0 transition-opacity hover:opacity-100 dark:bg-zinc-600" />
            </div>
          )}
            ...
            animate={
              isMobile
                ...
                : {
                    opacity: 1,
                    x: windowWidth
                      ? windowWidth * splitRatio
                      : `${splitRatio * 100}%`,
                    y: 0,
                    height: windowHeight,
                    width: windowWidth
                      ? windowWidth * (1 - splitRatio)
                      : `${(1 - splitRatio) * 100}dvw`,
                    borderRadius: 0,
